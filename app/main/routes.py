...
def send_message(recipient):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(msg)
        user.add_notification('unred_message_count',user.new_messages())
        db.session.commit()
...
@login_required
def  messages():
    current_user.last_message_read_time1 = datetime.utcnow()
    current_user.add_notification('unred_message_count',0)
    db.session.commit()
    page=request.args.get('page',1,type=int)
...
def export_posts():
    if current_user.get_task_in_progress('export_posts'):
        ...
        current_user.launch_task('export_posts', _('导出...'))
        db.session.commit()
